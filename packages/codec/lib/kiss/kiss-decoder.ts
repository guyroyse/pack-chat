...
/** Thrown when a KISS frame is missing its closing FEND — caller should buffer more bytes. */
export class IncompleteKISS_FrameError extends Error {
  constructor() {
    super('Incomplete KISS frame')
    this.name = 'IncompleteKISS_FrameError'
  }
}
...
export function decodeKISS_Frame(bytes: Uint8Array): [Uint8Array, Uint8Array] {
  ...
  if (state !== State.END_OF_FRAME) throw new IncompleteKISS_FrameError()
